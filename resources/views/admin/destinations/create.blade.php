                ...
                <div class="card-body">
                    <form method="post" action="{{ route('admin.destinations.store') }}">
                        ...
                        <div class="form-group {{ $errors->has('description') ? 'has-danger' : 'has-success' }}">
                            ...
                            <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : 'is-valid' }}" name="description" id="" cols="30" rows="10">{{ old('description', $destination->description) }}</textarea>
                        </div>

                        <div class="form-group  {{ $errors->has('location') ? 'has-danger' : 'has-success' }}">
                            <label class="mt-4 form-label">Location</label>    
                            <input class="form-control {{ $errors->has('location') ? 'is-invalid' : 'is-valid' }}" type="text" name="location" value="{{ old('location', $destination->location) }}">
                        </div>
